...
test("master motion system covers every page and adds visible media reveals",async()=>{
  ...
  assert.match(app,/preloadHeroImage\(heroSlides\[0\]\)/);
  assert.doesNotMatch(app,/heroSlides\.map\(preloadHeroImage\)/);
...
test("official Samsung guide stays consistent across languages and supports pinch zoom",async()=>{
  ...
  assert.match(app,/function measureLightboxImage/);
...
test("refined intro, magazine gallery, luggage media, room lock, and home-native OTA panel",async()=>{
  ...
  assert.match(html,/brandMarkSequence 1\.95s/);
  assert.match(html,/introSignalGrain \.34s/);
  assert.match(html,/introLogoSignalDissolve \.34s/);
  assert.match(html,/introSignalScan \.34s/);
  ...
  assert.match(app,/resolve\(\);setTimeout\(\(\)=>intro\.remove\(\),460\)/);
  assert.match(html,/\.brand-intro\.is-leaving\{[^}]*transition:opacity \.38s cubic-bezier\(\.4,0,\.2,1\) \.04s/);
  assert.match(html,/\.intro-home-zoom \.landing-hero \.hero-photo-primary\.is-active\{opacity:1;transition:none;animation:heroHomeRevealZoom 1\.7s/);
  ...
  assert.match(app,/classList\.remove\('intro-home-zoom'\),1800/);
